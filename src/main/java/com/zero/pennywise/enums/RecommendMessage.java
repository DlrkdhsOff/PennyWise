...
public enum RecommendMessage {
  ...
  EXPENSES_TOO_HIGH("평균 지출이 수입의 80%% 이상입니다. %,s의 지출을 줄이고 저축을 해보세요."),
  ...
  public static String getMessage(Long totalIncome, Long totalExpenses, Long savingAccount,
      String maxExpenseCategory) {
    StringBuilder sb = new StringBuilder();

    // 1. 수입이 너무 적을 경우
    if (totalIncome < 500000L) {
      return INCOME_TOO_LOW.generateMessage();
    }
    ...
    if (savingAccount == null || savingAccount == 0L) {
      ...
      if (recommendedSavings > totalIncome - totalExpenses) {
        sb.append(EXPENSES_TOO_HIGH.generateMessage(maxExpenseCategory)).append("\n");
      }
      sb.append(RECOMMENDED_SAVINGS.generateMessage(recommendedSavings)).append("\n")
          .append(ONE_YEAR_SAVINGS.generateMessage(recommendedSavings, recommendedSavings * 12));

      // 저축을 하고 있는 경우
    } else {
      // 저축이 추천 금액에 미치지 못하는 경우
      if (savingAccount < recommendedSavings) {
        sb.append(GOOD_PROGRESS.generateMessage(recommendedSavings - savingAccount)).append("\n")
            .append(ONE_YEAR_SAVINGS.generateMessage(recommendedSavings, recommendedSavings * 12));
      }
      // 저축이 추천 금액 이상인 경우
      else {
        sb.append(KEEP_GOING.generateMessage(recommendedSavings * 12)).append("\n");
      }
    }
